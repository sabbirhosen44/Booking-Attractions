...
# Splits raw review rows into (matched, skipped) against known attraction ids, and builds PropertyReviews rows plus SkipProperties rows for the skipped ones — same split as ReviewsImporter.process_file().
def build_property_reviews_frames(reviews_df, known_ids_df):
    ...
    reviews_out = matched.select(
        ...
        _truncate(F.coalesce(F.col("language"), F.lit("en")), 3).alias("language_code"),
